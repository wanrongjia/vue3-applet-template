// trailing参数用来控制是否最后一次是否执行，默认为true
const throttle = function throttle(fn, interval, leading = true, trailing = false) {
    // 该变量用于记录上一次函数的执行事件
    let lastTime = 0
    // 内部的控制是否立即执行的变量
    let isLeading = true
...
const debounce = function debounce(fn, delay = 1000, immediate = true, resultCb) {
